...
DROP TRIGGER TRI_USER_TB_user_no;
...
DROP SEQUENCE SEQ_USER_TB_user_no;
...
CREATE SEQUENCE SEQ_CALCULATE_TB_no INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_CLASS_TB_no INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_COMMENT_TB_no INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_COMMON_BOARD_TB_seq INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_PAYMENT_TB_no INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_REFUND_TB_no INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_RESERVATION_TB_no INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_TEACHER_TB_teacher_no INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_UPLOAD_TB_no INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_USER_TB_user_no INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_WISH_TB_no INCREMENT BY 1 START WITH 1;
...
(
	teacher_no number NOT NULL,
	user_no number,
	branch_no number(2,0) NOT NULL,
	profile_photo varchar2(300) NOT NULL,
...
(
	...
	image_width number,
	image_height number,
	file_dt date DEFAULT sysdate NOT NULL,
	delete_code varchar2(1) DEFAULT 'N' NOT NULL,
	file_delete_dt  date DEFAULT sysdate NOT NULL,
	PRIMARY KEY (no)
);
...
(
	...
	email_confirm number(1) DEFAULT 0 NOT NULL,
	activate_key number(1) DEFAULT 0 NOT NULL,
	PRIMARY KEY (user_no)
);
...
ALTER TABLE RESERVATION_TB
	...
	REFERENCES CUSTOMER_TB (user_no)
;
...
CREATE OR REPLACE TRIGGER TRI_USER_TB_user_no BEFORE INSERT ON USER_TB
FOR EACH ROW
BEGIN
	SELECT SEQ_USER_TB_user_no.nextval
	INTO :new.user_no
	FROM dual;
END;
...
COMMENT ON COLUMN ALTER_TB.teacher_no IS '강사 번호';
...
COMMENT ON COLUMN APPROVE_TB.user_no IS '회원 번호';
...
COMMENT ON COLUMN CUSTOMER_TB.user_no IS '고객 번호';
...
COMMENT ON COLUMN UPLOAD_TB.file_dt IS '첨부파일 등록일자';
...
COMMENT ON COLUMN USER_TB.user_no IS '회원 번호';
...
COMMENT ON COLUMN USER_TB.email_confirm IS '이메일인증 여부 : (0: X, 1: O)';
COMMENT ON COLUMN USER_TB.activate_key IS '활성화 상태 여부 : (0: X, 1: O)';
